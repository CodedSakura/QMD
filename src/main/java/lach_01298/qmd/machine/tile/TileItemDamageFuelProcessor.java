...
public class TileItemDamageFuelProcessor extends TileSidedInventory implements IItemProcessor, ITileGui<ProcessorUpdatePacket>
{
	...
	public TileItemDamageFuelProcessor(String name,  int itemInSize,int itemFuelSize, int itemOutSize, @Nonnull List<ItemSorption> itemSorptions, int time, boolean shouldLoseProgress, @Nonnull ProcessorRecipeHandler recipeHandler,@Nonnull ProcessorRecipeHandler fuelHandler, int processorID, int sideConfigYOffset)
	{
	...
	public void refreshFuel() 
	{
		...
		for(ItemStack fuel :getItemFuels())
		{
			ItemStack copy = fuel.copy();
			copy.setItemDamage(0);
			items.add(copy);
		}
		fuelInfo = fuelHandler.getRecipeInfoFromInputs(items,new ArrayList<Tank>());
	}
